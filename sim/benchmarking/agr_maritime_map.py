...
run = 0
available = []
while exists(result_folder + f"detections{run}.csv") or exists(
    result_folder + f"result{run}.csv"
):
...
# Robot detection positions and error lines, (un)comment as desired
"""if have_robot_pos:
    ax.scatter(
        robot_xs,
        robot_ys,
    ...
    for tx, ty, rx, ry in zip(target_xs, target_ys, robot_xs, robot_ys):
        ax.plot([tx, rx], [ty, ry], color="purple", linewidth=0.5, alpha=0.3, zorder=2)"""
...
ax.set_ylabel("Y (m)")
ax.set_title(
    f"Target belief vs reality - {len(available)} runs aggregated\n"
    f"({len(all_detections)} total detection events)"
)
ax.legend(loc="upper right", fontsize=8)
plt.tight_layout()

out = figures_folder + f"tracker_map_aggregated_{scenario}.png"
