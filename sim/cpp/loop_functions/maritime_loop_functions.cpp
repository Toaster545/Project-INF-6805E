...
CMaritimeLoopFunctions::CMaritimeLoopFunctions()
    : CLoopFunctions(), m_pcTargetLight(nullptr)
{
...
void CMaritimeLoopFunctions::Init(TConfigurationNode &t_tree)
{
    try
    {
        ...
        max_x = 9.5f;
        max_y = 9.5f;
        if (NodeAttributeExists(tMaritime, "min_x"))
            GetNodeAttribute(tMaritime, "min_x", min_x);
        if (NodeAttributeExists(tMaritime, "min_y"))
            GetNodeAttribute(tMaritime, "min_y", min_y);
        if (NodeAttributeExists(tMaritime, "max_x"))
            GetNodeAttribute(tMaritime, "max_x", max_x);
        if (NodeAttributeExists(tMaritime, "max_y"))
            GetNodeAttribute(tMaritime, "max_y", max_y);
